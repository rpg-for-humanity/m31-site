...
export default function ConvosetTest() {
  ...
  // Intro walking animation - Kokorobot walks to center
  useEffect(() => {
    if (gameState === 'intro') {
      setKokoroX(-150);
      setIsWalking(true);
      ...
      const walkIn = setInterval(() => {
        setKokoroX(prev => {
          // Walk to about 35% from left (center-left position)
          const target = window.innerWidth * 0.35;
          if (prev >= target) {
  ...
  const startGame = () => {
    ...
    const walkToCenter = setInterval(() => {
      setKokoroX(prev => {
        const target = window.innerWidth / 2 - 60;
        if (prev >= target) {
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      {/* Full Body Kokorobot - walks during intro and transition */}
      {showFullBody && (gameState === 'intro' || gameState === 'walking') && (
        ...
        >
          <img 
            ...
            alt="Kokorobot" 
            className={`h-64 w-auto drop-shadow-2xl ${isWalking ? 'animate-walk' : ''}`}
            style={{
      ...
      {gameState === 'intro' && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className={`text-center transition-all duration-500 ${introReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <p className="text-amber-200/90 mb-8 text-lg md:text-xl max-w-md mx-auto">
            ...
            <button
              onClick={startGame}
              disabled={!introReady}
              className="bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-black font-bold py-4 px-12 rounded-full text-xl transition transform hover:scale-105 shadow-lg shadow-amber-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Begin Mission
            </button>
          </div>
      ...
      {gameState === 'walking' && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <p className="text-amber-400 text-2xl animate-pulse font-medium">Kokorobot is approaching...</p>
        </div>
      )}
      ...
      <style jsx>{`
        ...
        .animate-walk {
          animation: walk 0.35s ease-in-out infinite;
        }
        @keyframes fade-in {
